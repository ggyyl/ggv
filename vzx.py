...
try:
    ...
    try:
        ...
        B = '\x1b[2;36m'
        Y = '\x1b[1;34m'
#        webbrowser.open('https://t.me/kke_o')
        P55 = pyfiglet.figlet_format("Hso ' ")
        ...
        kai = int(kai)
        ID = input(C + ' -  ID : ')
        webbrowser.open('https://vm.tiktok.com/ZSJWjoDkX/')
        print('\n')
        token = input(X + '  - Token : ')
        print('\n')
        print(C + '========================================')
        print('\n\n')
        e = '_'
        ktyp = '1696636424:AAF3cxn6zH3gtM1h6AwQoLMl5Qt5shLPeEQ'
            ...
            while True:
                ...
                if ku in kd:
                    ...
                    with open('user.txt', 'a') as (x):
                        x.write(ks + '\n')
                        tlg = f"https://api.telegram.org/bot{token}/sendMessage?chat_id={ID}&text= حسوني القائد جابلك يوزر يشك شك\n- - - - - - - - - - - - - - - - - - - - - - - - - -\n[=] user =>   ( {ks} )  ✓\n- - - - - - - - - - - - - - - - - - - - - - - - - -\n By :- @kke_o -: @Hseenj"
                else:
                    Number += 1
